...
pub(super) fn decode_head_timestamp(
...
pub(super) fn decode_historical_data(
    server_version: i32,
    time_zone: &Tz,
    message: &mut ResponseMessage,
...
pub(super) fn decode_historical_ticks_bid_ask(
    message: &mut ResponseMessage,
...
pub(super) fn decode_historical_ticks_mid_point(
    message: &mut ResponseMessage,
...
pub(super) fn decode_historical_ticks_last(
    message: &mut ResponseMessage,
...
pub(super) fn decode_historical_data_update(
    server_version: i32,
    time_zone: &Tz,
    message: &mut ResponseMessage,
) -> Result<Bar, Error> {
...
mod tests {
    ...
    fn test_decode_head_timestamp() {
        ...
        if let Ok(head_timestamp) = results {
            assert_eq!(
                head_timestamp,
                datetime!(2019-06-12 13:30).assume_utc(),
                "head_timestamp"
            );
    ...
    fn test_decode_historical_schedule() {
        ...
        if let Ok(schedule) = results {
            ...
            assert_eq!(
                ...
                datetime!(2023-04-14 16:00:00.0)
                    .assume_timezone(time_zone)
                    .unwrap(),
                "schedule.sessions[0].end"
            );
    ...
    fn test_decode_historical_data() {
        ...
        if let Ok(historical_data) = results {
            ...
            assert_eq!(
                historical_data.bars[0].open, 182.94,
                "historical_data.bars[0].open"
            );
            assert_eq!(
                historical_data.bars[0].high, 186.50,
                "historical_data.bars[0].high"
            );
            assert_eq!(
                historical_data.bars[0].low, 180.94,
                "historical_data.bars[0].low"
            );
            assert_eq!(
                historical_data.bars[0].close, 185.90,
                "historical_data.bars[0].close"
            );
            assert_eq!(
                historical_data.bars[0].volume, 948837.22,
                "historical_data.bars[0].volume"
            );
            assert_eq!(
                historical_data.bars[0].wap, 184.869,
                "historical_data.bars[0].wap"
            );
            assert_eq!(
                historical_data.bars[0].count, 324891,
                "historical_data.bars[0].count"
            );
        } else if let Err(err) = results {
    ...
    fn test_decode_historical_tick_bid_ask() {
        ...
        assert_eq!(
            ticks[0].timestamp,
            datetime!(2023-04-10 13:29:59 UTC),
            "ticks[0].timestamp"
        );
        assert_eq!(
        ...
        assert_eq!(
            ticks[3].timestamp,
            datetime!(2023-04-10 13:30:00 UTC),
            "ticks[0].timestamp"
        );
        assert_eq!(
    ...
    fn test_decode_historical_tick_last() {
        ...
        assert_eq!(
            ticks[0].timestamp,
            datetime!(2023-04-10 13:30:0 UTC),
            "ticks[0].timestamp"
        );
        assert_eq!(
        ...
        assert_eq!(
            ticks[0].special_conditions, " O X",
            "ticks[0].special_conditions"
        );

        assert_eq!(
            ticks[6].timestamp,
            datetime!(2023-04-10 13:30:02 UTC),
            "ticks[6].timestamp"
        );
        assert_eq!(
        ...
        assert_eq!(
            ticks[6].special_conditions, "   I",
            "ticks[6].special_conditions"
        );
    }
    ...
    fn test_decode_historical_tick_midpoint() {
        ...
        assert_eq!(
            ticks[0].timestamp,
            datetime!(2023-04-10 13:29:58 UTC),
            "ticks[0].timestamp"
        );
