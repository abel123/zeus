...
pub enum Error {
    ...
    ParseTimeRange(time::error::ComponentRange),
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Error::ServerVersion(wanted, have, message) => {
                write!(f, "server version {wanted} required, got {have}: {message}")
            }

            Error::Simple(ref err) => write!(f, "error occurred: {err}"),
            Error::ParseTimeRange(ref err) => err.fmt(f),
        }
    }
}
