...
use crate::element::chan::{GenericCache, BI, DT};
...
impl Matcher {
    pub fn is_match(&self, signals: Vec<Signal>) -> Option<(&Event, &Factor, DT)> {
        ...
        for event in &self.0 {
            ...
            let mut dt = Local::now().fixed_offset();
            let mut br = false;
...
mod tests {
    ...
    use crate::element::event::{Event, Factor, Operate, Signal};
