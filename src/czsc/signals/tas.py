...
def update_ma_cache(c: CZSC, **kwargs):
    ...
    assert (
        ma_type in ma_type_map.keys()
    ), f"{ma_type} 不是支持的均线类型，可选值：{list(ma_type_map.keys())}"
...
def update_boll_cache_V230228(c: CZSC, **kwargs):
    ...
    if cache_key not in last_cache.keys() or len(c.bars_raw) < timeperiod + 15:
        ...
        close = np.array([x.close for x in c.bars_raw])
        u1, m, l1 = ta.BBANDS(
            close, timeperiod=timeperiod, nbdevup=nbdev, nbdevdn=nbdev, matype=0
        )
...
def tas_boll_vt_V230212(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}BOLL{timeperiod}S{nbdev}MO{max_overlap}_BS辅助V230212".split(
        "_"
    )
    v1 = "其他"
    ...
    if _bars[-1].close > _bars[-1].cache[key]["上轨"] and any(
        [x.close < x.cache[key]["上轨"] for x in _bars]
    ):
        v1 = "看多"

    elif _bars[-1].close < _bars[-1].cache[key]["下轨"] and any(
        [x.close > x.cache[key]["下轨"] for x in _bars]
    ):
        v1 = "看空"
...
def tas_macd_base_V221028(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **参数列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}MACD{fastperiod}#{slowperiod}#{signalperiod}#{key}_BS辅助V221028".split(
        "_"
    )
...
def tas_macd_direct_V221106(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **参数列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}K#MACD{fastperiod}#{slowperiod}#{signalperiod}方向_BS辅助V221106".split(
        "_"
    )
...
def tas_macd_power_V221108(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}K#MACD{fastperiod}#{slowperiod}#{signalperiod}强弱_BS辅助V221108".split(
        "_"
    )
...
def tas_macd_first_bs_V221201(c: CZSC, **kwargs):
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}MACD{fastperiod}#{slowperiod}#{signalperiod}_BS1辅助V221201".split(
        "_"
    )
...
def tas_macd_first_bs_V221216(c: CZSC, **kwargs):
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}MACD{fastperiod}#{slowperiod}#{signalperiod}_BS1辅助V221216".split(
        "_"
    )
    bars = get_sub_elements(c.bars_raw, di=di, n=300)
...
def tas_macd_second_bs_V221201(c: CZSC, **kwargs):
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}MACD{fastperiod}#{slowperiod}#{signalperiod}_BS2辅助V221201".split(
        "_"
    )
...
def tas_macd_xt_V221208(c: CZSC, **kwargs):
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}K#MACD{fastperiod}#{slowperiod}#{signalperiod}形态_BS辅助V221208".split(
        "_"
    )
...
def tas_macd_bc_V221201(c: CZSC, **kwargs):
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}N{n}M{m}#MACD{fastperiod}#{slowperiod}#{signalperiod}_BCV221201".split(
        "_"
    )
    ...
    if len(bars) == n + m:
        ...
        if (
            n_macd[-1] > n_macd[-2]
            and min(n_close) < min(m_close)
            and min(n_macd) > min(m_macd)
        ):
            v1 = "底部"
        elif (
            n_macd[-1] < n_macd[-2]
            and max(n_close) > max(m_close)
            and max(n_macd) < max(m_macd)
        ):
            v1 = "顶部"
...
def tas_macd_change_V221105(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}K{n}#MACD{fastperiod}#{slowperiod}#{signalperiod}变色次数_BS辅助V221105".split(
        "_"
    )
...
def tas_ma_base_V230313(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{freq}_D{di}#{ma_type}#{timeperiod}MO{max_overlap}_BS辅助V230313".split("_")
    key = update_ma_cache(c, ma_type=ma_type, timeperiod=timeperiod)
    v1 = "其他"
    ...
    if bars[-1].close >= bars[-1].cache[key] and not all(
        x.close > x.cache[key] for x in bars
    ):
        v1 = "看多"
    elif bars[-1].close < bars[-1].cache[key] and not all(
        x.close < x.cache[key] for x in bars
    ):
        v1 = "看空"
    else:
...
def tas_ma_round_V221206(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}TH{th}#碰{ma_type}#{timeperiod}_BE辅助V221206".split("_")
    v1 = "其他"
    ...
    if len(c.bi_list) > di + 3:
        ...
        if (
            ...
            and abs(last_bi.high - last_ma) / bi_change < th / 100
        ):
            v1 = "上碰"
        elif (
            last_bi.direction == Direction.Down
            and abs(last_bi.low - last_ma) / bi_change < th / 100
        ):
            v1 = "下碰"
...
def tas_double_ma_V221203(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}T{th}#{ma_type}#{timeperiod1}#{timeperiod2}_JX辅助V221203".split(
        "_"
    )
    bars = get_sub_elements(c.bars_raw, di=di, n=3)
    ...
    **信号列表：**
    ...
    if bar.cache[cache_key1] > bar.cache[cache_key2] and bar.close < bar.open:
        ...
        for x in bars[::-1]:
            if x.cache[cache_key1] > x.cache[cache_key2]:
                right_bars.append(
                    {"bar": x, "大实体阴线": x.solid > solid_th and x.close < x.open}
                )
            else:
                break
    ...
    if bar.cache[cache_key1] < bar.cache[cache_key2] and bar.close > bar.open:
        ...
        for x in bars[::-1]:
            if x.cache[cache_key1] < x.cache[cache_key2]:
                right_bars.append(
                    {"bar": x, "大实体阳线": x.solid > solid_th and x.close > x.open}
                )
            else:
                break
...
def tas_boll_bc_V221118(c: CZSC, **kwargs):
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}N{n}M{m}L{line}#BOLL{timeperiod}_背驰V221118".split("_")
...
def tas_kdj_base_V221101(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}K#KDJ{fastk_period}#{slowk_period}#{slowd_period}_KDJ辅助V221101".split(
        "_"
    )
    bars = get_sub_elements(c.bars_raw, di=di, n=3)
...
def tas_first_bs_V230217(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    timeperiod = int(kwargs.get("timeperiod", 5))
    key = update_ma_cache(c, ma_type=ma_type, timeperiod=timeperiod)
    k1, k2, k3 = f"{c.freq.value}_D{di}N{n}#{ma_type}#{timeperiod}_BS1辅助V230217".split(
        "_"
    )
    v1 = "其他"
...
def tas_second_bs_V230228(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    timeperiod = int(kwargs.get("timeperiod", 20))
    key = update_ma_cache(c, ma_type=ma_type, timeperiod=timeperiod)
    k1, k2, k3 = f"{c.freq.value}_D{di}N{n}#{ma_type}#{timeperiod}_BS2辅助V230228".split(
        "_"
    )
    v1 = "其他"
    if len(c.bars_raw) < n + 5:
...
def tas_boll_cc_V230312(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}BOLL{timeperiod}S{nbdev}SP{sp}_BS辅助V230312".split("_")
...
def tas_macd_bs1_V230312(c: CZSC, **kwargs):
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}MACD{fastperiod}#{slowperiod}#{signalperiod}_BS1辅助V230312".split(
        "_"
    )
    v1 = "其他"
    ...
    if (
        ...
        and last_fx.raw_bars[-1].cache[cache_key]["macd"]
        > last_fx.raw_bars[0].cache[cache_key]["macd"]
    ):
        v1 = "看多"
...
def tas_macd_bs1_V230313(c: CZSC, **kwargs):
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{c.freq.value}_D{di}MACD{fastperiod}#{slowperiod}#{signalperiod}_BS1辅助V230313".split(
        "_"
    )
    bars = get_sub_elements(c.bars_raw, di=di, n=300)
...
def tas_macd_base_V230320(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{freq}_D{di}MACD{fastperiod}#{slowperiod}#{signalperiod}MO{max_overlap}#{key}_BS辅助V230320".split(
        "_"
    )
    v1 = "其他"
...
def tas_cci_base_V230402(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{freq}_D{di}CCI{timeperiod}#{min_count}#{max_count}_BS辅助V230402".split("_")
    v1 = "其他"
...
def tas_kdj_evc_V230401(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{freq}_D{di}T{th}{cache_key}#{key}值突破{min_count}#{max_count}_BS辅助V230401".split(
        "_"
    )
    v1 = "其他"
...
def tas_accelerate_V230531(c: CZSC, **kwargs) -> OrderedDict:
    ...
    if (
        all([x.close > x.cache[cache_key]["中线"] for x in bars])
        and up_zdf > t / 10 * mid_zdf > 0
    ):
        v1 = "多头加速"
...
def tas_cross_status_V230619(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{freq}_D{di}MACD{fastperiod}#{slowperiod}#{signalperiod}_金死叉V230619".split(
        "_"
    )
    v1 = "其他"
    if len(bars) >= 100:
...
def tas_rumi_V230704(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    (
        ...
        k2,
        k3,
    ) = f"{freq}_D{di}F{timeperiod1}S{timeperiod2}R{rumi_window}_BS辅助V230704".split("_")
    v1 = "其他"
...
def tas_macd_dist_V230408(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    q = pd.cut(
        factors, n, labels=list(range(1, n + 1)), precision=5, duplicates="drop"
    )[-1]
    ...
    if c1_macd[-1] > 0 and sum([1 if x > 0 else 0 for x in c1_macd]) != len(c1_macd):
        ...
        if len(c2_bars) > 3:
            ...
            for bar1, bar2 in zip(c2_bars, c2_bars[1:]):
                if (
                    bar1.cache[cache_key]["macd"] < 0
                    and bar2.cache[cache_key]["macd"] > 0
                ):
...
def tas_angle_V230802(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    same_dir_ang = [
        bi.power_price / bi.length for bi in bis[:-1] if bi.direction == b1.direction
    ][-n:]
...
def tas_macd_bc_V230804(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    if b5.direction == Direction.Up and b5.high > (gg - (gg - dd) / 4):
        ...
        od_dif = max(
            [x.cache[cache_key]["dif"] for x in b1.fx_b.raw_bars + b3.fx_b.raw_bars]
        )
        if 0 < b5_dif < od_dif:
            v1 = "空头"
...
def tas_macd_bc_ubi_V230804(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    if ubi["direction"] == Direction.Up and ubi["high"] > (gg - (gg - dd) / 4):
        ...
        od_dif = max(
            [x.cache[cache_key]["dif"] for x in b2.fx_b.raw_bars + b4.fx_b.raw_bars]
        )
        if 0 < b5_dif < od_dif:
            v1 = "空头"
