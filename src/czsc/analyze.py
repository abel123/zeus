...
class CZSC:
    ...
    def __update_bi(self):
        ...
        if self.verbose:  # and len(bars_ubi) > 100:
            logger.info(
                f"{self.symbol} - {self.freq} - {bars_ubi[-1].dt} 未完成笔延伸数量: {len(bars_ubi)}"
            )

        if envs.get_bi_change_th() > 0.5 and len(self.bi_list) >= 5:
            price_seq = [x.power_price for x in self.bi_list[-5:]]
            benchmark = min(
                self.bi_list[-1].power_price, sum(price_seq) / len(price_seq)
            )
        else:
            benchmark = None
        ...
        if self.verbose:  # and len(bars_ubi) > 100:
            logger.info(
                f"{self.symbol} - {self.freq} - {self.bars_ubi[-1].dt} 未完成笔延伸数量: {len(self.bars_ubi)}"
            )
        # 后处理：如果当前笔被破坏，将当前笔的bars与bars_ubi进行合并，并丢弃
        last_bi = self.bi_list[-1]
        bars_ubi = self.bars_ubi

        self.verbose and logger.info(f"last_bi {last_bi}, last bar {bars_ubi[-1]}")
    ...
    def to_plotly(self):
        ...
        kline.add_sma(
            df, ma_seq=(34, 55, 89, 144), row=1, visible=False, line_width=1.2
        )
        ...
        if len(bi_list) > 0:
            bi1 = [
                {"dt": x.fx_a.dt, "bi": x.fx_a.fx, "text": x.fx_a.mark.value}
                for x in bi_list
            ]
            bi2 = [
                {
                    "dt": bi_list[-1].fx_b.dt,
                    "bi": bi_list[-1].fx_b.fx,
            ...
            kline.add_scatter_indicator(
                fx["dt"], fx["fx"], name="分型", row=1, line_width=2
            )
            kline.add_scatter_indicator(
                bi["dt"], bi["bi"], name="笔", text=bi["text"], row=1, line_width=2
            )
        return kline.fig
    ...
    def last_bi_extend(self):
        ...
        if (
            self.bi_list[-1].direction == Direction.Up
            and max([x.high for x in self.bars_ubi]) > self.bi_list[-1].high
        ):
            return True

        if (
            self.bi_list[-1].direction == Direction.Down
