...
mod tests {
    ...
    fn test_encode_request_head_timestamp() {
        ...
        match results {
            Ok(message) => {
                ...
                assert_eq!(
                    message[2],
                    contract.contract_id.to_field(),
                    "message.contract_id"
                );
                assert_eq!(message[3], contract.symbol, "message.symbol");
                assert_eq!(
                    message[4],
                    contract.security_type.to_field(),
                ...
                assert_eq!(
                    message[10], contract.primary_exchange,
                ...
                assert_eq!(
                    message[14],
                    contract.include_expired.to_field(),
                    "message.include_expired"
                );
        ...
        match message {
            Ok(message) => {
                ...
                if contract.is_bag() {
                    assert_eq!(
                        message[i],
                        contract.combo_legs.len().to_field(),
                        "message.combo_legs_count"
                    );
                    i += 1;
                    ...
                    for combo_leg in &contract.combo_legs {
                        assert_eq!(
                            message[i],
                            combo_leg.contract_id.to_field(),
                            "message.contract_id"
                        );
                        i += 1;
                ...
                assert_eq!(
                    message[i],
                    keep_up_to_date.to_field(),
                    "message.keep_up_to_date"
                );
    ...
    fn test_encode_interval() {
        ...
        assert_eq!(
            valid_end_utc.to_field(),
            "20230415 10:00:00 UTC",
            "encode end utc"
        );
        assert_eq!(
            valid_end_ny.to_field(),
            "20230415 14:00:00 UTC",
            "encode end from America/NewYork"
        );
    }
    ...
    fn test_encode_request_historical_ticks() {
        ...
        let results = encode_request_historical_ticks(
            request_id,
            &contract,
            start,
            end,
        ...
        match results {
            Ok(message) => {
                assert_eq!(
                    message[0],
                    OutgoingMessages::RequestHistoricalTicks.to_field(),
                    "message.type"
                );
                assert_eq!(message[1], request_id.to_field(), "message.request_id");
                assert_eq!(
                ...
                assert_eq!(
                    message[17],
                    number_of_ticks.to_field(),
                    "message.number_of_ticks"
                );
