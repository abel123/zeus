...
def get_max_bi_num(v: int = None) -> int:
    ...
    默认值为 150，仅使用内置的信号和因子，不需要调整这个参数。
    如果进行新的信号计算需要用到更多的笔，可以适当调大这个参数。
    """
    max_bi_num = v if v else os.environ.get("czsc_max_bi_num", 150)
...
def get_bi_change_th(v: float = None) -> float:
    ...
    assert (
        2 >= bi_change_th >= 0.5 or bi_change_th == -1
    ), "czsc_bi_change_th not in [0.5, 2]"
    return bi_change_th
